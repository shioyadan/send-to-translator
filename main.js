...
const ID_CLIPBOARD_COPY = "ID_CLIPBOARD_COPY";
...
async function writeClipboard(text) {
    // Offscreen ドキュメントがなければ作る
    try {
        await chrome.offscreen.createDocument({
            url: "offscreen.html",
            reasons: [chrome.offscreen.Reason.CLIPBOARD],
            justification: "Copy selected text to clipboard"
        });
    } catch (e) {
        // 既に存在する場合はエラーになるので無視
    }
 ...
 * @param {string} lang
 * @param {string} text
 * @param {chrome.tabs.Tab} text
 */
function send(id, lang, text, tab) {
...
function main() {
    ...
    chrome.contextMenus.onClicked.addListener((info, tab) => {
        if (info.menuItemId == ID_DEEPL || info.menuItemId == ID_GOOGLE_TRANSLATE || info.menuItemId == ID_CLIPBOARD_COPY) {
            ...
            chrome.i18n.detectLanguage(text, (result) => {
                ...
                send(info.menuItemId, fromLang, text, tab);
            });
        }
    });
